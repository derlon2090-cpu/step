...
const schema = z.object({ modelId: z.string().regex(/^grammar-\d{2}$/), questionId: z.string().regex(/^grammar-\d{2}-q\d{2}$/), selectedIndex: z.number().int().min(0).max(3), clientMutationId: z.string().uuid().nullable().optional() });
export default async function handler(req, res) {
  if (req.method !== 'POST') return res.status(405).json({ error: 'Method Not Allowed' });
  try {
    const payload = schema.parse(typeof req.body === 'string' ? JSON.parse(req.body) : req.body ?? {});
    const result = await saveSourceAnswer((await requireUser(req)).user, { skill: 'grammar', questionSourceId: payload.questionId, selectedIndex: payload.selectedIndex, modelSourceId: payload.modelId, clientMutationId: payload.clientMutationId });
